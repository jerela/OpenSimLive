...
namespace OpenSimLive {
	...
	public:
		...
		std::array<double, 6> runTracker(const SimTK::State* s, OpenSim::Model* model, const std::string& bodyName, const std::string& referenceBodyName); // runs the PointTracker methods that calculate mirrored position and orientation, called from IMUInverseKinematicsToolLive
	...
	private:
		...
		SimTK::Vec3 calculatePointRotation(const SimTK::State* s, OpenSim::Model* model, const OpenSim::Body* body, const OpenSim::Body* referenceBody);
		...
		std::vector<std::array<double, 6>> timeSeriesDepData_; // a vector that will contain the outputs of PointTracker (mirrored point locations and rotations)
